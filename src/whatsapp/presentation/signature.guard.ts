...
export class SignatureGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const receivedDigest = Buffer.from(signatureHeader.slice(SIGNATURE_HEADER_PREFIX.length), 'hex');
    const expectedDigest = Buffer.from(this.computeDigest(request.rawBody), 'hex');
  ...
  private readSignatureHeader(request: RawBodyRequest): string {
    ...
    if (typeof header !== 'string' || !header.startsWith(SIGNATURE_HEADER_PREFIX)) {
